...
public class SIARDValidator {
  ...
  private void validateSubMets(Map<String, InputStream> subMets, boolean isZip) {
    ...
    while (var3.hasNext()) {
      ...
      } catch (JAXBException | SAXException | IOException var9) {
        Exception e = var9;
  ...
  private void validateRootMets() {
    try {
      ...
      String ipPath;
      if (this.structureValidatorState.isZipFileFlag()) {
        metsRootStream = this.structureValidatorState.getZipManager().getMetsRootInputStream(this.earksipPath);
        ipPath = this.siardSipPath.toString();
      } else {
        metsRootStream = this.structureValidatorState.getFolderManager().getMetsRootInputStream(this.earksipPath);
      ...
      this.metsValidatorState.setMets(metsRoot.instatiateMetsFile());
      this.metsValidatorState.setIpType(this.metsValidatorState.getMets().getMetsHdr().getOAISPACKAGETYPE());
      this.validateComponents();
    } catch (JAXBException | SAXException | IOException var6) {
      Exception e = var6;
      String message = this.createExceptionMessage(e, this.earksipPath.toString() + "/METS.xml");
      ReporterDetails csipStr0 = new ReporterDetails("CSIP-", message, false, false);
      csipStr0.setSpecification("CSIP-");
      ResultsUtils.addResult(this.validationReportOutputJson.getResults(), "CSIP0", csipStr0);
    }
